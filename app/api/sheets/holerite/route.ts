 ...
 * Busca e retorna todos os dados da aba 'Holerite'.
 */
export async function GET() {
    try {
        ...
        console.error(`[API Holerite GET] Erro: ${errorMessage}`);
 ...
 * Adiciona uma nova linha de dados na aba 'Holerite'.
 */
export async function POST(request: Request) {
    try {
        ...
        return NextResponse.json({
            success: true,
            message: 'Holerite adicionado com sucesso.',
            data: newRow.toObject()
        }, { status: 201 });

    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Um erro desconhecido ocorreu.';
        console.error(`[API Holerite POST] Erro: ${errorMessage}`);
