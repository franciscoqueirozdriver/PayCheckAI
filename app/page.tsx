...
const DEFAULT_ROWS: Payment[] = [
  {
    id: '1',
    dataPagamento: '2025-08-29',
    empresa: 'Driver Empresa 03',
    tipo: 'Mensalidade',
    parcela: '6 de 6',
    valorBruto: 7000,
    percImposto: 19,
  ...
  {
    id: '3',
    dataPagamento: '2025-08-30',
    empresa: 'Driver Empresa 01',
    tipo: 'Implantação',
    parcela: '2 de 2',
    valorBruto: 45000,
    percImposto: 19,
  ...
  {
    id: '5',
    dataPagamento: '2025-08-04',
    empresa: 'Driver Empresa 04',
    tipo: 'Implantação',
    parcela: '2 de 2',
    valorBruto: 20000,
    percImposto: 19,
...
export default function Page() {
  ...
  useEffect(() => {
    localStorage.setItem('dsr_aliquotas_globais', JSON.stringify(aliquotas));
  }, [aliquotas]);
  useEffect(() => {
    localStorage.setItem(
      'dsr_parametros',
      JSON.stringify({ ...params, bases })
    );
  }, [params, bases]);
  ...
  const holidays = useMemo(
    () =>
      params.considerarFeriados
        ? listHolidaysWrapper(params.uf, params.municipio)
        : [],
    [params.considerarFeriados, params.uf, params.municipio]
  );

  const diasUteisSemSabado = useMemo(
    () => countBusinessDays(days, { includeSaturday: false, holidays }),
    [days, holidays]
  );
  const diasUteisComSabado = useMemo(
    () => countBusinessDays(days, { includeSaturday: true, holidays }),
    [days, holidays]
  );
  ...
  const sundays = useMemo(() => countSundays(days), [days]);
  const feriadosDescanso = params.considerarFeriados
    ? holidays.filter((h) => {
        const dow = h.getDay();
        if (dow === 0) return false;
        if (!params.includeSaturday && dow === 6) return false;
        return true;
      }).length
    : 0;
  const diasDescanso = sundays + feriadosDescanso;

  const handlePeriodChange = (value: string) =>
    setParams((p) => ({ ...p, period: value }));
  ...
  return (
    <main className="max-w-7xl mx-auto pb-20">
      <HeaderStats
        ...
        diasUteisSemSabado={diasUteisSemSabado}
        diasUteisComSabado={diasUteisComSabado}
