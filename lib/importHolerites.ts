...
export function processHoleriteText(text: string, options: { userEmail?: string, fonte: string }): { extracted: HoleriteDraft, candidates: CandidatesMap } {
    ...
    // --- Header fields (Generic - Best effort since no sample was provided) ---
    ...
    // --- Specific patterns from user sample ONLY ---
    ...
    addNumericField('total_proventos', /Total de Vencimentos\s*:\s*R?\$\s*([0-9.,]+)/i, 1);
    addNumericField('total_descontos', /Total de Descontos\s*:\s*R?\$\s*([0-9.,]+)/i, 1);
    addNumericField('valor_liquido', /Valor Líquido\s*:\s*R?\$\s*([0-9.,]+)/i, 1);

    // --- Other fields and fallbacks ---
    ...
    if (!extracted.salario_base || extracted.salario_base === '0') {
        extracted.salario_base = String(findValue(rubricas, /dias normais/i));
    }
